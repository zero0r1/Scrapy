    ...
    ,'track10Search':'Track'}
            ...
            for node in airWayBillTrackingHistoryDtos:
               if 'Flight Arrived' in node.get('status') and ata == '':     #由于最新的数据在最顶端,所以第一次进来就是获取最新数据
                   ...
                   jsonData['ATA'] = ' at '.join((node.get('gateInDate'),node.get('gateInTime')))
                   ata = jsonData['ATA']
               ...
               if 'Flight Departed' in node.get('status'):
                   jsonData['ATD'] = ' at '.join((node.get('departedGateDate'),node.get('departedGateTime')))
            ...
            if countFlightNumber == currentFlightNumber:
                self.Insert('aacargo_analyze',json.dumps(jsonData))
                self.outputLog('save data to the data base')
                self.Insert('aacargo',masterAirWayBillDetailsSource)
